...
const PACKED_PART_SIZE = 16 * 1024;
...
function buildPayload(kind, original, hash) {
    ...
    for (let offset = 0; offset < compressed.length; offset += PACKED_PART_SIZE) {
        ...
        const partHash = crypto.createHash("sha256").update(packed).digest("hex").slice(0, 12);
        const fileName = `${kind.toLowerCase()}-${hash.slice(0, 20)}-${String(number).padStart(2, "0")}-${partHash}.psx`;
