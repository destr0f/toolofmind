...
const mainCompressed = compressLzss(mainBytes);
const windCompressed = compressLzss(windBytes);
assertRoundTrip("main", mainBytes, mainCompressed);
assertRoundTrip("WindUI", windBytes, windCompressed);

const mainPayload = encodeBase85(mainCompressed);
const windPayload = encodeBase85(windCompressed);
if (!decodeBase85ForVerification(mainPayload, mainCompressed.length).equals(mainCompressed)) {
    throw new Error("Main Base85 round-trip verification failed");
}
...
local __PSX_MAIN_PACKED_SIZE = ${mainCompressed.length}
...
__psxTrace(
    ...
    "version=" .. __PSX_LOADER_VERSION .. " | bytes=${mainCompressed.length + windCompressed.length}"
)
...
process.stdout.write(
    ...
    + `  Main:   ${mainBytes.length} -> ${mainCompressed.length} bytes\n`
);
