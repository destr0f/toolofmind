...
local function applyCoinData(rawId, data, fromEvent)
    ...
    record.MaxHealth = math.max(previousMax, maxHealth or 0, health or 0)
    if record.MaxHealth ~= previousMax then selectionChanged = true end
...
function coinIndex:IndexModel(model, refreshHealth)
    ...
    if refreshHealth and not record.FromServer then
        ...
        if health ~= nil then
            ...
            local previousMax = tonumber(record.MaxHealth) or 0
            record.MaxHealth = math.max(previousMax, health)
            if record.MaxHealth ~= previousMax then
                selectionChanged = true
            end
            if previousHealth ~= nil and ((previousHealth > 0) ~= (record.Health > 0)) then
                selectionChanged = true
...
local function refreshWorkspaceCoins()
    ...
    coinIndex.ScannedFolder = folder
    for _, model in ipairs(folder:GetChildren()) do
        coinIndex:IndexModel(model, true)
    end
end
...
function coinSync.UpdateFeedState()
    ...
    elseif coinSync.SignalConnections["New Coin"] then
        -- Structurally verified source (t4[1] OnClientEvent or exact t2[1]
        -- bridge). Connect alone never proves life; the first real event does.
        coinSync.FeedState = "BOUND_VERIFIED"
    else
        coinSync.FeedState = "UNAVAILABLE"
    end
end
...
local function connectCoinSignals(forceName)
    ...
    local function connect(name, callback)
        ...
        -- Only the game's own inbound route is a live feed: the t4[1] RemoteEvent
        -- OnClientEvent or the exact t2[1] inbound stand-in. The protected
        -- Fired accessor is never called from the injected thread: its
        -- anti-tamper returns an orphaned bindable that fakes a connected feed
        -- and starves the farm.
        local signal, source, routeProblem = coinSync.NetworkTransport:ResolveInbound(name)
        if not signal then
            local health = coinSync.SignalHealth[name] or { EventsSeen = 0 }
            health.Source = "unavailable: " .. tostring(routeProblem or source)
    ...
    connect("New Coin", function(id, data)
        ...
        local controller = coinSync.PetFarm
        if record and BossChestNames[normalize(record.Name)] == true then
            local selectedWorld, selectedZone = getSelectedWorld(), getSelectedZone()
            ...
            local directSource = string.sub(source, 1, 17) == "Network4 inbound "
            local health = coinSync.SignalHealth["New Coin"]
            if health then
            ...
            if selectedBoss and controller
                ...
                controller:HandleBossSpawn(record, source, directSource, data, true)
    ...
    connect("Update Coin Health", function(id, health)
        ...
        elseif (tonumber(health) or 0) > 0 then
            ...
            local controller = coinSync.PetFarm
            if record and not coinSync.SignalConnections["New Coin"]
                and controller and type(controller.HandleBossSpawn) == "function" then
...
function coinSync:RebindNewCoinSignal()
    ...
    -- Point invalidation only: a rebind must never flush the other live routes.
    if coinSync.NetworkTransport then
        pcall(coinSync.NetworkTransport.ClearRoute, coinSync.NetworkTransport,
...
function petFarm:ResolveBossPetRuntime(rawPetIds)
    ...
    if type(warp.NativePets) == "table" and warp.NativeScript
        ...
        for _, petId in ipairs(petIds) do
            local state = warp.NativePets[petId] or warp.NativePets[tonumber(petId)]
    ...
    warp.ResolveRetryAt = now + 2
    if type(getsenv) ~= "function" then
        warp.LastProblem = "getsenv unavailable; normal C54 path retained"
        return nil, warp.LastProblem
    end
    ...
    if not petsScript then
        warp.LastProblem = "Game.Pets LocalScript unavailable; normal C54 path retained"
        return nil, warp.LastProblem
    end

    local envOk, scriptEnv = pcall(getsenv, petsScript)
    if not envOk or type(scriptEnv) ~= "table" then
        warp.LastProblem = "Game.Pets environment unavailable; normal C54 path retained"
        return nil, warp.LastProblem
    end

    local callbacks = warp.CallbackScratch
    table.clear(callbacks)
    for _, name in ipairs({ "Tick", "NetworkUpdate", "ChangePetTargetCoin", "ChangePetTargetPlayer" }) do
...
function petFarm:WarpBossPetsOnce(record, rawPetIds, bossGeneration)
    ...
    local generationKey = tostring(record.Id) .. "@"
        .. tostring(bossGeneration or record.BossSpawnGeneration or coinMutationSerial)
    ...
    local primary = model and model:IsA("Model") and model.PrimaryPart
        or (model and model:IsA("BasePart") and model or nil)
    local visualTarget = model and (model:FindFirstChild("POS", true)
        or primary or model:FindFirstChild("Coin", true))
    if visualTarget and not visualTarget:IsA("BasePart") then visualTarget = nil end

    local baseCFrame
    if typeof(record.Position) == "Vector3" then
        baseCFrame = CFrame.new(record.Position)
    elseif typeof(record.Position) == "CFrame" then
        baseCFrame = record.Position
    elseif visualTarget then
        baseCFrame = visualTarget.CFrame
    end
    if not baseCFrame then
        warp.LastProblem = "boss position unavailable; normal C54 path retained"
        warp.Skipped = (tonumber(warp.Skipped) or 0) + 1
        return false
    end
    ...
    if type(nativePets) ~= "table" then
        warp.LastProblem = tostring(petIdsOrProblem)
        warp.Skipped = (tonumber(warp.Skipped) or 0) + 1
        return false
    end
    local petIds = petIdsOrProblem
    local radius = visualTarget and math.max(
        math.max(visualTarget.Size.X, visualTarget.Size.Z) * 0.55 + 3.5,
        4.5
    ) or 5.7
    local parts, cframes, states = warp.Parts, warp.CFrames, warp.States
    table.clear(parts)
    table.clear(cframes)
    table.clear(states)
    ...
    for index = 1, total do
        ...
        if typeof(physical) == "Instance" and physical:IsA("BasePart")
            ...
            cframes[#cframes + 1] = baseCFrame * CFrame.Angles(0, angle, 0)
    ...
    if #parts == 0 then
        warp.NativePets = nil
        warp.NativeScript = nil
        warp.LastProblem = "equipped pet parts unavailable; normal C54 path retained"
        warp.Skipped = (tonumber(warp.Skipped) or 0) + 1
        return false
    end
    ...
    for _, state in ipairs(states) do
        if visualTarget then
            state.target = visualTarget
            state.networkTarget = visualTarget
        end
        state.farming = true
        state.follower = nil
...
function coinSync.NetworkTransport:ResolveInbound(commandName)
    local controller, loadProblem = self:Ensure()
    local routeKey = "inboundEvent:" .. tostring(commandName)
    if not controller then
        self.UnresolvedRoutes[routeKey] = tostring(loadProblem)
        return nil, "Network4 adapter", tostring(loadProblem)
    end
    local ok, signal, source, _, problem = pcall(
        controller, "resolveInboundEvent", self.Context, commandName)
    if not ok then
        self.UnresolvedRoutes[routeKey] = tostring(signal)
        return nil, "Network4 adapter", tostring(signal)
    end
    if typeof(signal) ~= "RBXScriptSignal" then
        local text = tostring(problem or commandName .. " did not resolve to a live inbound signal")
        self.UnresolvedRoutes[routeKey] = text
        return nil, tostring(source or "Network4 inbound"), text
    end
    self.UnresolvedRoutes[routeKey] = nil
    self.ResolvedRoutes[routeKey] = tostring(source or "Network4 inbound")
    return signal, tostring(source or "Network4 inbound"), nil
end
...
function coinSync.NetworkTransport:ClearRoute(cache, commandName, remote)
    ...
    for _, candidate in ipairs(self:CommandRouteCandidates(commandName)) do
        ...
        self.ResolvedRoutes["inboundEvent:" .. candidate] = nil
    end
end
...
function petFarm:ConfirmStateProgress(state, source, now)
    ...
    local firstConfirmation = state.ProgressConfirmed ~= true
    local confirmedAt = tonumber(now) or os.clock()
...
function petFarm:RunProgressLeases(token)
    ...
    for petId, state in pairs(self.ProgressLeases) do
        ...
        local record = current and coinRecords[tostring(state.CoinId)] or nil
        local alive = current and recordAlive(record)
        if not current then
        ...
        elseif not alive then
            ...
            local observedHealth = tonumber(state.ProgressObservedHealth)
...
function petFarm:ConfirmCoinPets(rawCoinId, payload)
    ...
    for _, state in pairs(petStates) do
        if tostring(state.CoinId) == coinId and state.Generation == farmGeneration
            and self.ProgressLeases[tostring(state.PetId)] == state then
            interested = true
            break
        end
    end
...
function petFarm:EnsureEngine()
    ...
    local context = {
        ...
        OnFailed = function(petId, state, record, reason)
            ...
            local authoritativeBoss = config.Mode == "Boss Chest Only" and record
                and isBossChest(record) and coinSync.SignalConnections["New Coin"] ~= nil
            ...
            if authoritativeBoss then
                lastRecovery = rejected and "boss generation rejected" or "boss transport exhausted"
                driverStatus = rejected
                    and "boss generation stale; waiting for authoritative New Coin"
...
local function clearAssignments(sendBack, callback)
    ...
    for coinId, petSet in pairs(groups) do
        ...
        table.sort(petIds)
        if #petIds > 0 then
            jobs[#jobs + 1] = { CoinId = coinId, PetIds = petIds }
        end
    end
    ...
    -- yielding request for every old target at once.
    ...
    pumpReset = function()
        if finished then return end
        while active < 4 and head <= #jobs do
            local job = jobs[head]
            head = head + 1
            active = active + 1
        ...
        if head > #jobs and active == 0 then finish() end
    end
    pumpReset()
...
requestFarmReset = function(reason)
    ...
    if farmResetScheduled or farmResetRunning or not running() then return end
    farmResetScheduled = true
    task.defer(function()
        farmResetScheduled = false
        if not running() then return end
        if allocatorBusy then
            task.delay(0.01, function() requestFarmReset(farmResetReason) end)
            return
        end

        farmResetRunning = true
        driverStatus = "resetting: " .. farmResetReason
        clearAssignments(true, function()
...
function petFarm:PhaseCounts()
    local working, joining = 0, 0
    for _, state in pairs(petStates) do
        if state.Phase == "working" then working = working + 1 else joining = joining + 1 end
    end
    return working, joining
end
    ...
    local function checkBossLiveness()
        ...
        if config.Mode == "Boss Chest Only" and not farmResetRunning and petFarm.Engine then
            ...
            elseif orphanedBoss and now - (tonumber(farmWatch.LastRearmAt) or 0) >= 3
                ...
                farmWatch.LastRearmAt = now
                farmWatch.StallRecoveries = farmWatch.StallRecoveries + 1
                driverStatus = "boss lifecycle repaired; awaiting authoritative New Coin"
...
function lootCollector:Ensure()
    if self.Controller then return true end
    if self.Loading then return false, "loot reactor is loading" end
    self.Loading = true
    local controller, problem = loadRemoteController("lootReactor", "loot reactor")
    self.Loading = false
    if not controller then
        self.Problem = tostring(problem)
        return false, self.Problem
...
function lootCollector:Status()
    if not self.Controller then
        return self.Problem and ("Loot reactor unavailable: " .. self.Problem)
            or "Native loot reactor is ready on demand."
    end
    local called, stats = pcall(self.Controller, "stats")
    if not called or type(stats) ~= "table" then
...
function lootCollector:StartWorker()
    ...
    local ready, problem = self:Ensure()
    if not ready then
    ...
    if called and accepted == true then
        ...
        self.Problem = tostring(called and startProblem or accepted)
        statusSetters.Set("Loot", "Loot reactor start failed: " .. self.Problem)
    end
end
...
function lootCollector:SyncWorker()
    if not self.StartupArmed then return end
    if not self:IsEnabled() then
        if self.WorkerActive then self:StopWorker() end
        return
    end
    if not self.WorkerActive then
        self:StartWorker()
        return
    end
    local called, accepted = pcall(self.Controller, "sync")
    if not called or accepted ~= true then
        self.WorkerActive = false
        self:StartWorker()
    end
...
UI.FarmHero:Dropdown({
    ...
    Callback = function(value)
        ...
        config.Mode = value
        petFarm:ResetBossPetWarp("assignment mode changed")
...
UI.FarmHero:Toggle({
    ...
    Title = "Boss Pet Instant Arrival",
    Desc = "After one accepted grouped Join, locally moves the equipped batch to the chest position. No extra remote or polling.",
    Value = false,
    Callback = function(value)
        config.BossPetInstantArrival = value == true
        petFarm:ResetBossPetWarp(config.BossPetInstantArrival
            and "armed for next accepted boss" or "disabled")
    end,
})
...
UI.WorldDropdown = UI.TargetSection:Dropdown({
    ...
    Callback = function(value)
        ...
        if changed and config.PetFarm then
            requestFarmReset("world selection changed")
            restartFarmWatchers()
        end
    end,
})
...
UI.GraphicsHero:Toggle({
    ...
    Title = "Farm Anti-Lag",
    Desc = "Processes each new farm effect once; rejoin to restore visuals already simplified",
    Value = false,
...
local function shutdown(reason)
    ...
    for _, entry in pairs(machineModules) do
        if type(entry) == "table" then entry.Controller = nil end
    end
    moduleLoadState.Busy = false
    moduleLoadState.Owner = nil
...
function requestDiagnostics.UpdateTelemetry()
    ...
    if lootStats then
        ...
        requestDiagnostics.Gauge("Loot", "bagTransportCommitted", tonumber(lootStats.BagTransportCommitted) or 0)
    ...
    local function tick()
        ...
        if monitorVisible then
            local networkState = (networkReady() and "ready" or "waiting")
                .. " | feed: " .. tostring(coinSync.FeedState or "UNBOUND")
            local bossStats = petFarm:BossStats()
            local bossLine = ""
            local warpLine = ""
            ...
            if config.BossPetInstantArrival then
                ...
                warpLine = string.format(
                    "\nWarp post-Join: batches/pets/skipped/errors %d/%d/%d/%d | %s",
            ...
            statusSetters.Farm(string.format(
                ...
                math.min(workingCount + joiningCount, equippedCount),
                equippedCount,
                workingCount,
                joiningCount,
